...
function TribunalComments({ contentId, reportStats }) {
  ...
  return (
    <Container>
      <TabNav pill>
        <TabNavItem onClick={() => onTabClick(All)} title="All" id={0} activeTab={key} setActiveTab={setKey} contentCount={allComments?.length || reportStats?.numAllComments} />
        <TabNavItem onClick={() => onTabClick(Defendant)} title="Defendant" id={1} activeTab={key} setActiveTab={setKey} contentCount={defendentComments?.length || reportStats?.numDefendantComments} />
        <TabNavItem onClick={() => onTabClick(Reporter)} title="Reporters" id={2} activeTab={key} setActiveTab={setKey} contentCount={reporterComments?.length || reportStats?.numReporterComments} />
        <TabNavItem onClick={() => onTabClick(Jury)} title="Jury" id={3} activeTab={key} setActiveTab={setKey} contentCount={juryComments?.length || reportStats?.numJuryComments} />
        <TabNavItem onClick={() => onTabClick(General)} title="General" id={4} activeTab={key} setActiveTab={setKey} contentCount={generalComments?.length || reportStats?.numGeneralComments} />
